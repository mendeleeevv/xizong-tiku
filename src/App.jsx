...
const STORAGE = {
  ...
  answers: 'xizong-b-answers',
  submitted: 'xizong-b-submitted',
...
export default function App() {
  ...
  const [bAnswers, setBAnswers] = useState(() => loadJSON(STORAGE.answers, {}))
  const [bSubmitted, setBSubmitted] = useState(() => loadJSON(STORAGE.submitted, {}))
  ...
  useEffect(() => saveJSON(STORAGE.answers, bAnswers), [bAnswers])
  useEffect(() => saveJSON(STORAGE.submitted, bSubmitted), [bSubmitted])
  ...
  const updateBSelection = useCallback((groupId, qi, key) => {
    if (bSubmitted[groupId]) return
    setBAnswers(prev => {
      const groupAnswers = prev[groupId] || {}
      const current = groupAnswers[qi] || []
      const next = current.includes(key) ? current.filter(k => k !== key) : [...current, key].sort()
      return { ...prev, [groupId]: { ...groupAnswers, [qi]: next } }
    })
  }, [bSubmitted])

  const submitBGroup = useCallback((item) => {
    const { group, id } = item
    const groupAnswers = bAnswers[id] || {}
    setProgress(prev => {
      const next = { ...prev }
      group.questions.forEach((q, qi) => {
        const user = (groupAnswers[qi] || []).join('')
        next[q.id] = user === q.answer ? 'correct' : 'wrong'
      })
      return next
    })
    setBSubmitted(prev => ({ ...prev, [id]: true }))
  }, [bAnswers])

  const redoBGroup = useCallback((item) => {
    const { group, id } = item
    setBAnswers(prev => {
      const next = { ...prev }
      delete next[id]
    ...
    setProgress(prev => {
      const next = { ...prev }
      group.questions.forEach(q => delete next[q.id])
      return next
    })
  }, [])
  ...
  const groupSubmitted = current ? Boolean(bSubmitted[current.id]) : false
  const groupAnswers = current ? (bAnswers[current.id] || {}) : {}
      ...
      <div className="layout">
            ...
            <>
              ...
              {current.group.type === 'b_type' ? (
                <BTypeGroup
                  ...
                  onSelect={updateBSelection}
                  onSubmit={submitBGroup}
                  onRedo={redoBGroup}
                />
              ) : (
        ...
        <aside className={`right-panel ${rightPanelOpen ? 'open' : ''}`}>
          ...
          {rightPanelTab === 'evidence' && (
            <EvidenceTab item={current} submitted={groupSubmitted} answers={groupAnswers} />
          )}
...
function BTypeGroup({ item, progress, answers, submitted, onSelect, onSubmit, onRedo }) {
  const { group, id } = item
  const hasOptions = group.sharedOptions && group.sharedOptions.length > 0
  const resultCount = submitted ? group.questions.filter((q, qi) => (answers[qi] || []).join('') === q.answer).length : 0

  return (
    <div className={`study-grid ${hasOptions ? '' : 'no-options'}`}>
      {hasOptions && (
        <aside className="option-bank option-rail">
          <div className="section-label"><span>共用选项</span><em>B型题</em></div>
          <div className="option-grid">
            {group.sharedOptions.map(opt => (
              <div className="shared-option" key={opt.id}><b>{opt.id}</b><span>{opt.text}</span></div>
            ))}
          </div>
          <p className="option-rail-hint">选项固定在此处，右侧题干按字母作答。</p>
        </aside>
      )}
      ...
      <div className="question-side">
        <section className="question-card">
          <div className="question-card-top">
            <span className="question-type">B型题</span>
            <span>题组 {item.groupIdx + 1} / {item.section.groups.length}</span>
            {group.sourcePage ? <span>来源页 {group.sourcePage}</span> : null}
          </div>
          <div className="stem-list">
            {group.questions.map((q, qi) => {
              ...
              const correct = submitted && sel.join('') === q.answer
              const missed = submitted && answer.some(k => !sel.includes(k))
              return (
                <div key={q.id} id={`q-${q.id}`} className={`stem-row ${submitted ? (correct ? 'is-correct' : 'is-wrong') : ''}`}>
                  <div className="stem-main">
                    <span className="stem-number">{String(qi + 1).padStart(2, '0')}</span>
                    <div className="stem-copy">
                      <div className="stem-heading">
                        <p>{q.text}</p>
                        <span className={`answer-mode ${multi ? 'is-multi' : ''}`}>{multi ? '多选' : '单选'}</span>
                      </div>
                      {!submitted && multi && <small>可选多个共用选项</small>}
                    </div>
                  </div>
                  <div className="answer-choices">
                    {group.sharedOptions.map(opt => {
                      const active = sel.includes(opt.id)
                      const isAnswer = submitted && answer.includes(opt.id)
                      const isMissed = submitted && isAnswer && !active
                      return (
                        <button
                          key={opt.id}
                          className={`answer-chip ${active ? 'active' : ''} ${submitted && isAnswer ? 'answer' : ''} ${isMissed ? 'missed' : ''} ${submitted && active && !isAnswer ? 'wrong' : ''}`}
                          onClick={() => onSelect(id, qi, opt.id)}
                          disabled={submitted}
                          aria-label={`选项 ${opt.id}`}
                        >
                          {opt.id}
                        </button>
                      )
                  ...
                  {submitted && (
                    <div className={`result-line ${correct ? 'ok' : 'bad'}`}>
                      <Icon name={correct ? 'check' : 'alert'} size={15} />
                      {correct ? '正确' : `讲解/原题答案：${q.answer.split('').join('、')}`}
                      {missed && <span className="missed-legend">橙色 = 漏选</span>}
                      {q.note && <span className="correction-dot">{q.note}</span>}
                    </div>
                  )}
                </div>
              )
          ...
          <div className="question-card-bottom">
            {submitted ? (
              <div className="submit-summary"><Icon name="check" size={18} /><span>已提交 · {resultCount} / {group.questions.length} 个题干正确</span></div>
            ) : (
              <span className="hint-text">完成每个题干后提交；多选按共用选项组作答。</span>
            )}
            <button className={`primary-button ${submitted ? 'redo-button' : ''}`} onClick={() => submitted ? onRedo(item) : onSubmit(item)}>
              {submitted ? '重新作答' : '提交本组'} <Icon name={submitted ? 'right' : 'arrow'} size={17} />
            </button>
          </div>
        </section>
      </div>
    ...
    <div className="evidence-body">
      <div className="evidence-section">
        ...
        <div className="evidence-facts">
          ...
          <div className="source-line"><span>原题页</span><strong>{group.sourcePage ? `第 ${group.sourcePage} 页` : '暂无页码'}</strong></div>
        </div>
      </div>

      <div className="evidence-section correction-section">
        <div className="evidence-title">
          <span className="correction-icon"><Icon name="alert" size={18} /></span>
          <div><h2>口诀与易错点</h2><p>{notes.length ? `${notes.length} 条记忆提示` : '本组暂无额外提示'}</p></div>
        </div>
        <div className="correction-body">
          {notes.length === 0 && <p>题目数据里没有附带的记忆口诀，可以点顶部的「笔记」自己补充。</p>}
          {notes.map((q, i) => (
            <div className="manual-note" key={q.id}>
              <strong>Q{i + 1} · 题目</strong>
              <p>{q.text}</p>
              {q.note && <p className="memory-point"><em>记忆点</em>{q.note}</p>}
            </div>
          ))}
          {isB && <div className="source-button static"><Icon name="eye" size={16} />原题第 {group.sourcePage} 页</div>}
        </div>
      </div>
